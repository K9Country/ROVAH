...
type SiteReviewsButtonProps = {
  hasPendingReviews: boolean;
  onPress: () => void;
};

export function SiteReviewsButton({ hasPendingReviews, onPress }: SiteReviewsButtonProps) {
  const pulse = useSharedValue(0);

  useEffect(() => {
    cancelAnimation(pulse);
    pulse.value = hasPendingReviews
      ? withRepeat(withSequence(withTiming(1, { duration: 900 }), withTiming(0, { duration: 900 })), -1, false)
      : withTiming(0, { duration: 160 });
    return () => cancelAnimation(pulse);
  }, [hasPendingReviews, pulse]);
  ...
  return (
    <Pressable
      accessibilityHint="Opens completed site visits that are ready for your review."
      accessibilityLabel={hasPendingReviews ? 'Site Reviews. A review is ready.' : 'Site Reviews'}
      accessibilityRole="button"
      onPress={onPress}
    >
      <Animated.View style={[styles.button, animatedStyle]}>
        <Text style={styles.label}>Site Reviews</Text>
        {hasPendingReviews ? <Text style={styles.notice}>Ready</Text> : null}
      </Animated.View>
    </Pressable>
  );
}
