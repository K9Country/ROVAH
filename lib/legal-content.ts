...
export const legalDocuments: LegalDocument[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { heading: 'Cancellations', paragraphs: ['Guests and hosts can cancel a standard, non-subscription reservation until one hour before its scheduled start time. After that one-hour cutoff, the reservation cannot be cancelled in the app. Subscription reservations cancelled before the cutoff restore the used visit credit to the guest’s active subscription.'] },
      { heading: 'Payment release and subscription credits', paragraphs: ['For a standard, non-subscription reservation, ROVAH secures the guest’s card at confirmation but Stripe does not collect payment until one hour before the visit starts. A cancellation before that cutoff releases the card authorization: the guest is not charged and the host earns no payout. Subscription purchases are not refunded through a reservation cancellation; instead, an eligible cancelled subscription visit restores its used visit credit.'] },
