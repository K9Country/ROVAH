-- A host has one private conversation with each guest, regardless of which
-- property they visit. Match the existing unique constraint so broadcasts can
-- reuse that conversation or create it when necessary.
...
begin
  ...
  where conversations.host_id = calling_host_id
