-- Keep the existing database identifiers for compatibility while presenting
-- the member- and host-facing offer as a Special Discount.
...
begin
  ...
  if property_name is null then
    raise exception 'Only the host of this property can send a Special Discount';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Special Discounts can only be sent to a completed member profile';
  end if;

  if p_discount_percent is null or p_discount_percent <= 0 or p_discount_percent > 82 then
    raise exception 'Special Discounts must be between 1%% and 82%%';
  end if;
  ...
  values (
    ...
    (select auth.uid()),
    format(
      'A %s%% Special Discount has been sent to you for %s. It will be available when you make your next reservation for this private space and can be used once through %s.%s',
...
revoke execute on function public.issue_resolution_discount(uuid, uuid, numeric, date, text) from anon;
