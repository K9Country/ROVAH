...
export default function PropertyDetailsScreen() {
  ...
  const reserveSpace = async () => {
    ...
    try {
      setIsBooking(true);
      const { data: bookingRows, error } = await supabase.rpc('create_booking_with_dogs', {
        p_property_id: property.id,
        p_start_at: startTime.toISOString(),
        p_end_at: endTime.toISOString(),
        p_dog_profile_ids: selectedDogIds,
      });
      const data = Array.isArray(bookingRows) ? bookingRows[0] : bookingRows;
      if (error) {
        if (error.code === '23P01') {
          await loadListing();
          setStartTime(null);
      ...
      if (!data?.id) throw new Error('The reservation could not be created. Please try again.');
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.bookingCard}>
          ...
          <View style={styles.estimateRow}><Text style={styles.estimateLabel}>Rental Fee</Text><Text style={styles.estimateValue}>${estimatedTotal.toFixed(2)}</Text></View>
            <Pressable disabled={property.is_temporarily_closed || isBooking || isDogProfilesLoading} onPress={reserveSpace} style={[styles.bookingButton, (property.is_temporarily_closed || isBooking || isDogProfilesLoading) && styles.buttonDisabled]}>{isBooking ? <ActivityIndicator color={colors.warmWhite} /> : <Text style={styles.bookingButtonText}>{!bookingDate ? 'Choose a date to continue' : !startTime ? 'Choose a start time to continue' : !endTime ? 'Choose an end time to continue' : !selectedDogIds.length ? 'Select attending dogs to continue' : 'Confirm Reservation'}</Text>}</Pressable>
        </View>
      </ScrollView>
